CREATE OR REPLACE FUNCTION keplersc.transfer_rollback(dataxml xml)
 RETURNS TABLE(resultado text, mensaje text, adicionales text)
 LANGUAGE plpgsql
AS $function$
DECLARE 
	--Esta funcion procesa los tipos de documentos X, D ( Pagos - Transfers ) a Comprobarse (Cuentas por Pagar por Comprobar)
	--Se controlara con un TAG en el XML y se procesara bajo el mismo concepto del Convert del Contrarecibo Universal 
	...
	--Autor: Jose Mendoza , 17 Abril 2024
	-- * * * Control de Cambios :
	--Autor: Jose Mendoza , 14 Mayo 2024 
	--Adaptar Cambios para Cambios de Estado {Comprobacion} de Documentos 
...
begin
	...
	*/
	...
	if upper(flag_gastos) <> 'CXP_TRANSFER_CONVERT' then
		raise exception '%', 'Se esta llamando a la funcion [ transfer_rollback ] desde una Operacion No Valida ...';
	end if;
	...
	if length(folio_operacion) = 0 then
		mensajeError := 'No se pudo obtener el Folio del Documento a Evaluar (Actualizar) ...';
		raise exception '%',mensajeError;
	end if;
	...
	*/
	...
	where c1 = sucursal_id and c2 = genero and c3 = naturaleza and c4 = grupo::int and c5 = tipo::int
		...
		and c11 = referencia and st_x_comprobar = 'T';
	if totalReg = 0 then
	...
	where w.c1 = sucursal_id and w.c2 = genero and w.c3 = naturaleza and w.c4 = grupo::int and w.c5 = tipo::int
		...
		and c11 = referencia and st_x_comprobar = 'V';
	
	if not found then 
		mensajeError := 'No se encontro Registro en KDM1 de la CxP Comprobada (Actualizada) ... ';
		raise exception '%', mensajeError;
	else
		mensajeError := '';
		if rec1.st_x_comprobar <> 'V'/*'X'*/ then
	...
	for intCont in 0..no_partidas - 1 loop
		...
		prov_pago := coalesce((xpath('//document/k_mov/r'||intCont||'/k_clave/text()',dataxml))[1],'');
		nombre := coalesce((xpath('//document/k_mov/r'||intCont||'/k_nombreprov/text()',dataxml))[1],'');
		...
		partida := coalesce((xpath('//document/k_mov/r'||intCont||'/partida/text()',dataxml))[1],'');	
		...
		if length(nombre) > 0 then no_vacios := no_vacios + 1; end if;
		...
		if length(partida) > 0 then no_vacios := no_vacios + 1; end if;

		if no_vacios < 7 and no_vacios > 0 then 
		...
		if no_vacios > 0 then
			...
			*/
			...
			if length(st_del) > 0 and upper(st_del) = upper('X') then 
				...
				if totalReg = 0 then
					mensaje := 'No se encontro el Registro en kdm5 [Partida] [' || partida || ']' 
...
--raise exception 'transfer_rollback 02';
	...
	-- For Testing ...
	/*raise exception '%','El Documento {Transfer} sera Convertido como Validado ...';*/
...
exception
	when others then
		resultado := 0;
		mensaje := 'transfer_rollback() ' || '['|| sqlstate || '] ' || sqlerrm;
		adicionales := '';
		return query select resultado, mensaje, adicionales;
