...
begin
	...
	get_resultado := '';
	get_adicionales := '';
--raise exception 'docdis_xa, PASO 1 %',dataxml;
	--Documento
	...
	if genero <> 'X' or naturaleza <> 'A' then --Cuentas por pagar, Acreedora	
		return;
	end if;
...
--raise notice '%',xmlKDM1;
	...
	then 
		...
		--Victor Salgado. Se agrega validacion para el manejo de impuestos
		if  (xpath('//row/c6/text()', xmlKDMM))[1]::text = 'S' and --Tiene movtos contables
			((xpath('//row/c71/text()', xmlKDMM))[1]::text = 'S' or concepto_factura <> '') then --Tiene movtos contables en pantalla
--raise exception 'vcss docdis_xa 1.1 concepto_factura % ',concepto_factura;
...
--raise exception 'vcss docdis_xa 2';		
		---------------------------------------------------------------
