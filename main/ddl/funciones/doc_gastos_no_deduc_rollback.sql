CREATE OR REPLACE FUNCTION keplersc.doc_gastos_no_deduc_rollback(dataxml xml)
 RETURNS TABLE(resultado text, mensaje text, adicionales text)
 LANGUAGE plpgsql
AS $function$
DECLARE 
	--Esta funcion procesa los tipos de documentos X, A Convertidos (Cuentas por Pagar por Des-Convertir)
	--No pasara por docdis, no se creara Docmento en KDM1 ni Cuenta por Pagar KDUXG, KDUXE 
	--pero estas Tablas seran Actualizadas Removiendo los Datos asociados en la Operacion 
...
begin
	...
	if upper(flag_gastos) <> 'CXP_CONTR_REC_CONVERT_NO_DEDUC_ROLLBACK' then
		raise exception '%', 'Se esta llamando a la funcion [ doc_gastos_no_deduc_rollback ] desde una Operacion No Valida ...';
	end if;
	...
	totalReg := 0;
	select count(*) into totalReg from keplersc.kdmm where col_sucursal = sucursal_id and /*added by JMM 20240925*/ c1=genero and c2=naturaleza and c3=grupo::int and c4=tipo::int;
	if totalReg = 0 then
	...
	*/
	...
	where c1 = sucursal_id and c2 = genero and c3 = naturaleza and c4 = grupo::int and c5 = tipo::int
		and c6 = folio_operacion and c10 = proveedor and c11 = referencia /*refanterior*/ and upper(st_x_comprobar) = 'N' /*'X'*/ /*'S'*/
		/*Commented by JMM 20240926 /*Added by JMM 20240724*/ and doc_refer_compl = refanterior*/;
	if totalReg = 0 then
		/*mensajeError := 'Documento a Comprobar No encontrado en la BD o No tiene el Formato Correcto ...';*/
		mensajeError := 'Documento a Revertir como No Deducible No encontrado en la BD o No tiene el Formato Correcto ...';
		raise exception '%',mensajeError;			
	end if;
	...
	paso:= 'doc_gastos_no_deduc_rollback.actualizar_DOC';
	...
	---------------------------------------------------------------
	--CONTABILIDAD. ALTA_CONT_DOC_CONVERT_NO_DEDUC_ROLLBACK
	---------------------------------------------------------------
	paso:= 'doc_gastos_no_deduc_rollback.alta_cont_doc_convert_no_deduc_rollback';
	select * into resultado, mensaje, adicionales from keplersc.alta_cont_doc_convert_no_deduc_rollback(dataxml,xmlKDM1,xmlKDMM,folio_operacion);
	if resultado = '0' then
	...
	--Resuelve: Similar a ALTA_CONT_SEC pero en BAJA_CONT_DOC_NO_DEDUC
	---------------------------------------------------------------
	paso:= 'doc_gastos_no_deduc_rollback.baja_cont_doc_no_deduc';
	select * into resultado, mensaje, adicionales from keplersc.baja_cont_doc_no_deduc(dataxml, folio_operacion);
	if resultado = '0' then
		raise exception '%', mensaje;
	end if;
	...
	where c1 = sucursal_id and c2 = genero and c3 = naturaleza and c4 = grupo::int and c5 = tipo::int
		and c6 = folio_operacion and c10 = proveedor and c11 = referencia /*refanterior*/ and upper(st_x_comprobar) = 'N' /*'X'*/ /*'S'*/ 
		/*Commented by JMM 20240927 /*Added by JMM 20240724*/ and doc_refer_compl = refanterior*/;
	...
	if not found then 
		...
		*/
		--Added by JMM 20240514
		if rec1.c11 <> referencia /*refanterior*/ then
			mensajeError := mensajeError || ' | ' || 'Referencia';
		end if;
		if rec1.st_x_comprobar <> 'S' /*'N'*/ /*'X'*/ then
	...
	*/ -- END : Commented Segment
	...
	-----------  START : Section KDM6 (Reverted ... Validating Budget ST ) Added by JMM 20240826

	count6 := 0;

	for rec6 in 
		select k1.c9 as fech,k1.c6 as fol,k1.c43 as st,k6.c8,k6.c9,k6.c10,k6.c11,k6.c13,k6.c12,k6.ctopto from keplersc.kdm6 k6
		...
		where k6.c1 = sucursal_id and k6.c2 = genero and k6.c3 = naturaleza and k6.c4 = grupo::int and k6.c5 = tipo::int and k6.c6 = folio_operacion 
			and k1.c10 = proveedor and k1.c11 = /*refanterior*/ referencia and upper(k1.st_x_comprobar) = 'S'
	loop
		
		count6 := count6 + 1;
		
		------  START : Segment for Budgets 
	
		st_prspto_concept := rec6.ctopto; 
		
		fecha := rec6.fech;
	
		numero_partida := count6;
	
		clave_cuenta := rec6.c8;
	
		asiento := rec6.c10;
	
		--For Testing
		/*raise exception '%', 'Fecha : ' || fecha || ' , Concepto Ppto : ' || st_prspto_concept;*/			
			...
			if upper(func_Valor) = 'S' then -- Cuenta Maneja Presupuesto
				...
				if upper(left(func_Valor,5)) = 'ERR -' then
					...
					if upper(left(func_Valor,5)) = 'ERR -' then
						...
						--Adapted by JMM 20240826 , this apply only for this specific case 
						/*if cargo <> '' then*/
		...
		------  END : Segment for Budgets 		

	end loop;

	if count6 = 0 then 
		raise exception '%', 'No se encontro el Documento Revertido para validar los Montos de Presupuestos (si aplicare el caso) ...';
	else
		--For Testing ...
		--raise exception '%', 'Se Valido el Documento Revertido en la parte de Presupuestos : ' || folio_operacion || ' , con [ ' || count6 || ' ] partidas';
	end if;
	

	-----------  END : Section KDM6 (Reverted ... Validating Budget ST ) Added by JMM 20240826
	...
	-- N.A. para este Tipo de Documento
	/* -- START : Commented Segment 20240926
	...
	*/
	...
	where m.c1 = sucursal_id and m.c2 = 'X' and m.c3 = 'D' and m.cve_prov_oper = proveedor and m.c14 = /*refanterior*/ referencia
		and w.c16 > 0 
		and ( (m.c4::int = 31 and m.c5::int = 1 and upper(w.c43) <> 'C') or (m.c4::int = /*33*/31 and m.c5::int = /*1*/11 and upper(m.st_x_comprobar) <> 'N') );
	...
	*/ -- END : Commented Segment 20240926 

	-----------  END : Section KDM5 Added by JMM 20240514


	-- For Testing ...
	--raise exception '%','El Documento sera Revertido en su Conversion como No Deducible ...';
...
exception
	when others then
		resultado := 0;
		mensaje := 'doc_gastos_no_deduc_rollback() ' || '['|| sqlstate || '] ' || sqlerrm;
		adicionales := '';
		return query select resultado, mensaje, adicionales;
