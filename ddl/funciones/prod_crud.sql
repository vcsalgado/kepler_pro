...
begin 
	...
	precio2 := coalesce((xpath('//document/input_precio2/text()', dataxml))[1]::text,'0')::integer;
	precio3 := coalesce((xpath('//document/input_precio3/text()', dataxml))[1]::text,'0')::integer;
	precio4 := coalesce((xpath('//document/input_precio4/text()', dataxml))[1]::text,'0')::integer;
...
exception
	when others then
		...
		mensaje := 'ref_crud() ' || '['|| sqlstate || '] ' || sqlerrm ;		
