CREATE OR REPLACE FUNCTION keplersc.verify_invalta(dataxml xml)
 ...
 LANGUAGE plpgsql
AS $function$
--Descripcion: Función en MOVLIB AUTOS  
--Autor: Saltiel Rc
--Fecha: 20/10/2022
...
begin
				...
				v_estatus := (xpath('//document/k_status_validacion/text()', dataxml))[1]; --siempre se inicializa en 1 para realizar validaciones --véase lib MOVLIB
 ...
 --raise exception 'v_operacion:%',v_operacion;
				if v_operacion = 'G' then --FACTURA					
					if strValor::int < 80 then --'VALIDA TODO EL PROCESO DE VENTA MIENTRAS NO SEA TRASPASO						
						...
						adicionales := 'No se puede realizar esta operaciÃ³n, porque la compra todavia no esta dada de alta'; raise exception '%',adicionales;
						end if;
						...
						if strValor::int > 20 and v_estado_venta < 10 then 
						adicionales := 'No se puede realizar esta operaciÃ³n, porque el pedido todavia no esta dado de alta'; raise exception '%',adicionales;
						end if;
						...
						if strValor::int > 30 and v_estado_venta < 20 then 
						adicionales := 'No se puede realizar esta operaciÃ³n, porque la factura todavia no esta dada de alta'; raise exception '%',adicionales;
						end if;
					...
					*/	 							 
					...
					--MSS 010523 Código faltante en la función
					...
					--MSS Código faltante hasta aquí
					end if;
