CREATE OR REPLACE FUNCTION keplersc.cita_crud(dataxml xml)
...
--				   Permitir alta de otra cita tipo G o M- Garantia y tener 2 citas activas una de otro tipo y una G o M
...
if origen = 'BPR' then
	...
	if tipo_operacion <> 2 then
		...
		if found then
			if (tipo_cita <> 'G' and tipo_cita <> 'M' and tipo_cita <> 'C') and tipo_operacion = 0 then		--MSS Permitir alta de otra cita tipo G o M- Garantia o C-Campanas y tener 2 citas activas una de otro tipo y una G
				message := concat('El numero de SERIE tiene otra cita que no ha sido Concretada con Folio: ',fol_cita_activa, ', Imposible Continuar. Solo se permite una cita de Garantia (G,M)');
			...
			if found then
				if (tipo_cita <> 'G' and tipo_cita <> 'M' and tipo_cita <> 'C') and tipo_operacion = 0 then		--MSS Permitir alta de otra cita tipo G o M- Garantia o C-Campanas y tener 2 citas activas una de otro tipo y una G
					message := concat('Estas placas tienen otra cita que no ha sido Concretada con Folio: ',fol_cita_activa, ', Imposible Continuar. Solo se permite una cita de Garantia (G,M)');
					raise exception '%', message;
				end if;
			end if;
		end if;
		...
		if kodawari = 'S' and origen <> 'BPR' then --VCSS Interfaces BP
			...
			if validar_cts_recep= 10 and (tipo_cita='N' or (validaciones_recep='S' and tipo_cita <> 'N')) then 
				...
				if found then 
					...
					--or c12=fecha_promesa_entrega and c13=LEFT(hora_promesa_entrega::text,5)		--18/09/23 MSS se quita validacion fecha promesa de entrega a peticion �rea de citas Celaya
					);
		...
		if tipo_operacion = 1 /*and reprogramacion = 1*/ then  
		...
		for cont in 0..no_puntos - 1 loop
			...
			and horas = ''  and precio_punto = ''  and clave_operario = '' then  
				...
				if tipo_cita = 'C' then
					...
					if tipo_cita <> 'G' and tipo_cita <> 'M' then		--MSS 14112025 Agregar manejo de campanas para ordenes de garantia G o M			
						if clave_campana <> '' then
							raise exception '%', format('Error en punto numero %1$L. Solo las Citas C,G o M pueden tener campañas.', numero_punto);
						end if;
					end if;
				end if;
				...
				if cita_en_linea <> 'S' then --LGLG 15/01/24 modulo citas en linea
					...
					*/
					...
					while hrs_ini < hrs_fin loop
						...
						-- * * * Cmnts by JMM : AQUI ESTA TRONANDO  ... Sugiero cambiar la forma de determinar la variable registro y datos ...
						-- El Error se debe a como se estan manejando los llamados a la funcion {keplersc.tmkt_control_de_citas} 
