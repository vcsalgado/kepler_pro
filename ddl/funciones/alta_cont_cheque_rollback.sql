CREATE OR REPLACE FUNCTION keplersc.alta_cont_cheque_rollback(dataxml xml, xmlkdm1 xml, xmlkdmm xml, folio_operacion text)
...
begin
	...
	if xpath_exists('//document/ambiente/schema/text()', dataxml) = true /*false*/ then 
		...
		raise exception '%', 'No se pudo Validar el Esquema para utilizar la Funcion Contable de Cheque Rollback ...';
	end if;
	-- Adapted Condition by JMM 20240516 
	if upper(flag_gastos) <> 'CXP_CHEQUE_BAJA' then
		raise exception '%', 'Esquema No Permitido para utilizar la Funcion Contable de Cheque Rollback ...';
	end if;
	...
	if totalReg = 0 then
		mensaje := 'No se encontraron Registros a Procesar en kdm5 ... CT Cheque Rollback';
	...
	*/
	...
	loop
		...
		nombre_cteprov := 'BAJA: ' || nombre_cteprov;
		nombre_cteprov := left(nombre_cteprov,40);
...
exception
	when others then
		resultado := 0;
		mensaje := 'alta_cont_cheque_rollback() ' || '['|| sqlstate || '] ' || sqlerrm;
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
end;
$function$
