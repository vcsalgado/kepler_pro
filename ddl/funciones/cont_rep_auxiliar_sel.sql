CREATE OR REPLACE FUNCTION keplersc.cont_rep_auxiliar_sel(dataxml xml)
 RETURNS xml
 LANGUAGE plpgsql
AS $function$
declare 
...
begin	
	...
	create temp table tmpkdc2 as select * from keplersc.kdc22301;
...
--raise notice 'Usuario %',usuario_movto;	
	...
	expSql:=format('select c8 as k_tipo_poliza, c3 as k_numero_cuenta,  
		desc_cuenta as k_desc_cuenta, c1 as k_n_poliza, 
		c2 as k_fecha_poliza, c7 as k_ref_poliza, c14 as k_suc_poliza,
		c15||c16||lpad(c17::text,2,%1$L)||lpad(c18::text,3,%1$L)||c19 as k_folio_poliza,
		desc_movto as k_desc_mov, 
		c6 as k_desc_poliza,usuario as k_usuario, 
		saldo_inicial as k_saldo_ini,cargos as k_cargo, abonos as k_abono,
		saldo_final as k_saldo_fin,orden 
		from tmpkdc2 order by orden','0');
--		from tmpkdc2 order by c3,c2,c1,c8','0');	

	select query_to_xml(expSql,false,true,'') into xmlAuxiliar;	
	
	/*	
	select query_to_xml('select c3 as cuenta, c8 as tipo_poliza, c1 as no_poliza, 
		c2 as fecha, c6 as desc_poliza, c14 as sucursal, 
		c15||c16||c17::text||c18::text||c19 as documento,
		saldo_inicial,cargos, abonos,saldo_final 
		from tmpkdc2 order by c3,c2,c8,c1',false,true,'') into xmlAuxiliar;
*/
	return xmlAuxiliar;
exception
	when others then
		error := 'keplersc.cont_rep_auxiliar_sel() ' || '['|| sqlstate || '] ' || sqlerrm ;
