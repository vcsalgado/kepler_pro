...
--				if rec_T_ICOM.c41 <> 'DR' then
--					continue;
--				end if;
--raise notice 'TOMAS Vendedor:%; Inventario:%;',rec_F_UV.c2,rec_T_ICOM.c2 ;
...
--raise notice 'PASO 1';
					...
					if rec_E_INF.c3=rec_H_VESQ.c14 then  --IF E3=H14 then Clave vehiculoraise notice 'PASO 1';
--raise notice 'PASO 2';
...
--raise notice 'rec_F_UV.c2:%; B11_ComisionSinBonos:%; rec_T_ICOM.c2:% ',rec_F_UV.c2, B11_ComisionSinBonos,rec_T_ICOM.c2;
...
--raise notice 'BONO TOMAS Vendidas Sucursal:%, c41:% A2_fechaIniMes:%, dateValor:%',sucursal_id,rec_F_UV.c2,A2_fechaIniMes,dateValor;		
			for rec_V_COMISMOV2 in select * from keplersc.kdcomismov2 k  --INI Loop 
				...
				order by c1,c12,c7
			loop
				select * into rec_E_INF from keplersc.kdinf where c1=rec_V_COMISMOV2.c1	and c2=rec_V_COMISMOV2.c8 limit 1;
				if not found then
raise notice 'No se encontro: %',rec_V_COMISMOV2.c8;
					continue;
				end if;
