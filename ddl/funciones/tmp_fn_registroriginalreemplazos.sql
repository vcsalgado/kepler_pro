CREATE OR REPLACE FUNCTION keplersc.tmp_fn_registroriginalreemplazos()
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
...
--											where c1=tmpCadena and c3='';
										str_reemp=str_reemp||''''||tmpCadena||''',';
										cuentareemp=cuentareemp+1;
										select sum(c5),sum(c6)
											into cantentradas,cantsalidas
											from keplersc.kdinl
											where c2=tmpCadena;
										raise notice '% REEMPLAZO cantentradas:%, cantsalidas:%',
										cuentareemp,cantentradas,cantsalidas;
										if cantentradas-cantsalidas>0 then
											contrex=contrex+1;
											raise notice '% Reemplazo con existencias:%-%=%',contrex,cantentradas,cantsalidas,cantentradas-cantsalidas;
											str=str||''''||tmpCadena||''',';
										else 
											if cantentradas-cantsalidas=0 then
												contrsex=contrsex+1;
												raise notice '% Sin existencias:%-%=%',contrsex,cantentradas,cantsalidas,cantentradas-cantsalidas;
												str_s=str_s||''''||tmpCadena||''',';
											else
												contrnull=contrnull+1;
												raise notice '% Existencias null o negativas:%-%=%',contrnull,cantentradas,cantsalidas,cantentradas-cantsalidas;
												str_n=str_n||''''||tmpCadena||''',';
											end if;
...
raise notice '% Total originales como reemplazos',cuentareemp;
raise notice '%',str_reemp;
raise notice '% Con existencias',contrex;
raise notice '%',str;
raise notice '% Sin existencias',contrsex;
raise notice '%',str_s;
raise notice '% Existencias null o negativas',contrnull;
raise notice '%',str_n;
