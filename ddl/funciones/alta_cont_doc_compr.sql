CREATE OR REPLACE FUNCTION keplersc.alta_cont_doc_compr(dataxml xml, folio_operacion text)
...
begin
	...
	for cont in 0..no_partidas - 1 loop
			...
			/*if upper(flag_contrarec) = 'CXP_CONTR_REC_CONVERT' then*/ -- Moved and evaluated to UP 20240404
			...
			insert into keplersc.kdmdocscompr (c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c13 ,c12/*Added by JMM 20240308*/ ,ctopto/*Added by JMM 20240723*/)
...
exception
	when others then
		resultado := 0;
		mensaje := 'alta_cont_doc_compr() ' || '['|| sqlstate || '] ' || sqlerrm ;
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
end;
$function$
