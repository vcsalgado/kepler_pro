...
DECLARE
	...
	--RHFC se incluyen variables para extraer sucursal y folio del json
	extracted_sucursal text;
	extracted_folio text;
	extracted_tipo int;
	jason_id_trans jsonb;
	datosInterfaz jsonb;
	--RHFC Termina se incluyen variables para extraer sucursal, tipo y folio del json
	--RHFC Termina se incluyen variables para extraer sucursal y folio del json
...
begin
    ...
    -- Seleccionar función según OEM:
    IF strOem = 'GWM' then
        SELECT * INTO resultado
        FROM keplersc.notif_registrar_movto_gwm(NEW, entidad, operacion);
    ELSIF strOem = 'GM' THEN
		IF entidad = 'IFZ_SOFIA_NOTIF' THEN
			SELECT * INTO resultado
        	FROM keplersc.notif_registrar_movto_gm_sofia(NEW, entidad, operacion);
		ELSE
			SELECT * INTO resultado
        	FROM keplersc.notif_registrar_movto_gm_crm(NEW, entidad, operacion);
		END IF;
        
	--RHFC INICIO
    ELSIF strOem = 'TOY' THEN
        SELECT * INTO resultado
        FROM keplersc.notif_registrar_movto_toyota(NEW, entidad, operacion);
	--RHFC FIN
    ELSE
    ...
    IF resultado.procesar = 'S' THEN
		...
		IF strOem = 'TOY' THEN
			...
			IF entidad = 'KDPEDREF' THEN
				INSERT INTO keplersc.notif_api_control_envios
		            (id_transaccion, api_id, datos_interfaz, sucursal, genero, naturaleza, grupo, tipo, folio)
		        VALUES
		            (intTransaction, resultado.api_id, resultado.datos_interfaz, extracted_sucursal,'N','D',21,1,extracted_folio);	
			ELSIF entidad = 'IFZ_DDOA_RDR_PREENVIO' THEN
				INSERT INTO keplersc.notif_api_control_envios
		            (id_transaccion, api_id, datos_interfaz, sucursal, genero, naturaleza, grupo, tipo, folio)
		        VALUES
		            (intTransaction, resultado.api_id, resultado.datos_interfaz, extracted_sucursal,'U','D',6,extracted_tipo,extracted_folio);
			ELSE
				INSERT INTO keplersc.notif_api_control_envios
		            (id_transaccion, api_id, datos_interfaz)
		        VALUES
		            (intTransaction, resultado.api_id, resultado.datos_interfaz);
			END IF;
			IF resultado.interfaz = 'DDOA'  THEN
				json_notif := jsonb_build_object(
	            	'schema_version','1.0',
					'cirdan_metadata', jsonb_build_object(
	                'id_transaccion',intTransaction
	            	),
	            	'target_workflow', jsonb_build_object(
	                	'config_key',resultado.api_id
	            	),
	            	'initial_context', jsonb_build_object(
	                	'params',resultado.datos_interfaz
	            	)
	        	);
				expSql := format('notify interfaces_toyota, %L', json_notif::text);
	        	EXECUTE expSql;
	
	        	RAISE NOTICE 'JSON enviado: %', json_notif;
			ELSE
		...
		ELSIF strOem = 'GM' THEN
			...
			IF entidad = 'IFZ_SOFIA_NOTIF' THEN
				INSERT INTO keplersc.notif_api_control_envios
	            	(id_transaccion, api_id, datos_interfaz, id_sofia_notif)
	        	VALUES
	            	(intTransaction, resultado.api_id, resultado.datos_interfaz, p_new.id_sofia_notif);
	        ...
	        EXECUTE expSql;
			RAISE NOTICE 'notify interfaces_gm enviado: %', intTransaction;
