CREATE OR REPLACE FUNCTION keplersc.alta_cont_doc_convert_no_deduc(dataxml xml, xmlkdm1 xml, xmlkdmm xml, folio_operacion text)
...
begin
	...
	*/
	...
	if upper(flag_contrarec) <> 'CXP_CONTR_REC_CONVERT_NO_DEDUC' then 
		raise exception '%', 'Se esta llamando a la funcion [ alta_cont_doc_convert_no_deduc ] desde una Operacion No Valida ...';
	else
		var_st_compr = 'N' /*'X'*/;
	end if;
	...
	if str_monto_iva > 0 then 
		...
		if not found then 
			...
			-- Condition UPD 20240702 by JMM, to implement a STD GAP for Decimals 
			if pagos > 0 and abs ( iva_acreditado - round( (recg.c6 * iva_factor) / (100 + iva_factor) , 2) ) > .10 then
				mensajeError := 'Discrepancia en la Conversion de IVA en KDUXG de la CxP Comprobada ... ';
				raise exception '%', mensajeError;
			end if;
	...
	*/ -- END : Commented Segment
	...
	-- Segment Adapted by JMM 20240923 
	/*
	...
	*/
	...
	if partidas_bef <> 1 then
		mensajeError := 'El Documento Origen [Kdm6] No es elegible para convertirse como No Deducible ...';
		raise exception '%',mensajeError;
	else
		select * into recp from keplersc.kdm6  
		where c1 = sucursal_id and c2 = genero and c3 = naturaleza and c4 = grupo and c5 = tipo_clave and c6 = folio_operacion;
		if not found then 
			mensajeError := 'El Documento Origen [Kdm6] a convertirse, No fue encontrado en la BD ... ';
			raise exception '%', mensajeError;
		else
	...
	--For Test
	/*raise exception '%', 'Cuenta Abono : ' || cuenta_abono_param;*/
	...
	if (xpath('//row/c6/text()', xmlKDMM))[1]::text  = 'S' and (xpath('//row/c71/text()', xmlKDMM))[1]::text  = 'S' then 
		...
		loop 
			...
			select xmlforest(fecha_operacion as fecha, clave_cuenta as cuenta, tipo_asiento_kdc as tipo_asiento, 
				...
				, var_concept_prspto as var_concept_prspto /*Added by JMM 20240718*/)::text into strValor;
		...
		if partidas_new <> 1 then 
			raise exception '%', 'El Documento No es Elegible para la Conversion como No Deducible, cuenta con ' || partidas_new || ' Partidas ' ;
		end if;
		...
		if str_monto_iva > 0 then
			...
			if iva_acreditado > 0 then
				...
				numero_partida_poliza_kdc = numero_partida_poliza_kdc + 1;
				-- CONT(T,W9,M21,B8091,W14,W32,W11,M18,"","","","","",W1...W6,B8095)
				select xmlforest(fecha_operacion as fecha, clave_cuenta/*cuenta_contable_iva*/ as cuenta, 'A' as tipo_asiento, 
				...
				numero_partida_poliza_kdc = numero_partida_poliza_kdc + 1;
				-- CONT(T,W9,M21,B8091,W14,W32,W11,M18,"","","","","",W1...W6,B8095)
				select xmlforest(fecha_operacion as fecha, clave_cuenta/*cuenta_contable_iva*/ as cuenta, 'C' as tipo_asiento, 
					iva_acreditado/*str_monto_iva*/ as monto,cuenta_dscr/*nombre_cteprov*/ as descrip, referencia as refer, 
					tipo_poliza_kdmm as tipo_poliza,'' as moneda, '' as depto, '' as concepto, '' as proyecto,
					sucursal_id as sucursal, genero, naturaleza, grupo, tipo_clave, folio_operacion,
					accion_poliza_kdc as accion_poliza, folio_poliza,
...
exception
	when others then
		resultado := 0;
		mensaje := 'alta_cont_doc_convert_no_deduc() ' || '['|| sqlstate || '] ' || sqlerrm;
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
end;
$function$
