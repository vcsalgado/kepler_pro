CREATE OR REPLACE FUNCTION keplersc.cat_exteriores_crud(dataxml xml)
 RETURNS TABLE(resultado text, mensaje text, adicionales text)
 LANGUAGE plpgsql
AS $function$
--Descripcion: Realiza crud del catalogo de exteriores KDICE2
...
begin 
	...
	if crud = 'MODIFICAR' then
		update keplersc.kdice2  
			set c4=descripcion 
			where c1=cve_modelo and c3=cve_color;
	end if;

	if crud = 'ELIMINAR' then
		delete from keplersc.kdice2 
			where c1=cve_modelo and c3=cve_color;
	end if;
...
exception
	when others then
		resultado := 0;
		mensaje := 'cat_exteriores_crud() ' || '['|| sqlstate || '] ' || sqlerrm ;		
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
