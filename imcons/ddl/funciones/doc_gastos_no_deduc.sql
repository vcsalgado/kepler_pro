CREATE OR REPLACE FUNCTION keplersc.doc_gastos_no_deduc(dataxml xml)
 RETURNS TABLE(resultado text, mensaje text, adicionales text)
 LANGUAGE plpgsql
AS $function$
DECLARE 
	--Esta funcion procesa los tipos de documentos X, A a Convertirse como No Ducibles (Cuentas por Pagar por Comprobar)
	--No pasara por docdis, no se creara Docmento en KDM1 ni Cuenta por Pagar KDUXG, KDUXE 
	--tampoco estas Tablas seran Actualizadas { KDUXG, KDUXE } con los Datos del XML asociado en la Operacion 
...
begin
	...
	if upper(flag_gastos) <> 'CXP_CONTR_REC_CONVERT_NO_DEDUC' then
		raise exception '%', 'Se esta llamando a la funcion [ doc_gastos_no_deduc ] desde una Operacion No Valida ...';
	end if;
	...
	totalReg := 0;
	select count(*) into totalReg from keplersc.kdmm where col_sucursal = sucursal_id and /*added by JMM 20240926*/ c1=genero and c2=naturaleza and c3=grupo::int and c4=tipo::int;
	if totalReg = 0 then
	...
	*/
	...
	if totalReg = 0 then
		mensajeError := 'Documento a Convertir No encontrado en la BD o No tiene el Formato Correcto ...';
		raise exception '%',mensajeError;			
	end if;
	...
	set st_x_comprobar = 'N' /*'X'*/, 
		...
		grupo_id = case when length(clave_gpogasto) = 0 then null else clave_gpogasto::integer end /*Added by JMM 20240829*/
	where c1 = sucursal_id and c2 = genero and c3 = naturaleza and c4 = grupo::int and c5 = tipo::int
		and c6 = folio_operacion and c10 = proveedor and c11 = referencia /*refanterior*/ and upper(st_x_comprobar) = 'S';
	...
	where w.c1 = sucursal_id and w.c2 = genero and w.c3 = naturaleza and w.c4 = grupo::int and w.c5 = tipo::int
		and w.c6 = folio_operacion and w.c10 = proveedor and c11 = referencia and upper(st_x_comprobar) = 'N' /*'X'*/  
		/* /*Added by JMM 20240723*/ and doc_refer_compl = refanterior -- Commented by JMM 20240923*/;
	...
	if not found then 
		...
		*/
		if rec1.st_x_comprobar <> 'N' /*'X'*/ then
	...
	paso:= 'doc_gastos_no_deduc.actualizar_DOC';
	...
	*/ -- END : Commented Segment
	...
	---------------------------------------------------------------
	--MOVIMIENTOS. Registro de movimientos contables en kdmdocsnodeduc 
	--Resuelve: Similar a ALTA_CONT_SEC pero en ALTA_CONT_DOC_NO_DEDUC 
	---------------------------------------------------------------
	paso:= 'doc_gastos_comprobar.alta_cont_doc_compr';
	select * into resultado, mensaje, adicionales from keplersc.alta_cont_doc_no_deduc(dataxml, folio_operacion);
	if resultado = '0' then
		raise exception '%', mensaje;
	end if;

	-- TO TEST 	
	/*raise exception '%', 'FUN [alta_cont_doc_no_deduc] Ejecutada ...';*/
	

	---------------------------------------------------------------
	--CONTABILIDAD. ALTA_CONT_DOC_CONVERT_NO_DEDUC
	---------------------------------------------------------------
	paso:= 'doc_gastos_comprobar.alta_cont_doc_convert_no_deduc';
	select * into resultado, mensaje, adicionales from keplersc.alta_cont_doc_convert_no_deduc(dataxml,xmlKDM1,xmlKDMM,folio_operacion);
	if resultado = '0' then
		raise exception '%', mensaje;
	end if;
	---------------------------------------------------------------
	...
	*/ -- END : Commented Segment
	...
	-- For Testing ...
	/*raise exception '%','El Documento sera Convertido como No Deducible ...';*/
...
exception
	when others then
		resultado := 0;
		mensaje := 'doc_gastos_no_deduc() ' || '['|| sqlstate || '] ' || sqlerrm;
		adicionales := '';
		return query select resultado, mensaje, adicionales;
