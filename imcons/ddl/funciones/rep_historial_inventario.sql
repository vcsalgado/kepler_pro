...
--Fecha: 23/11/2022
--Bitacora de cambios
--10/04/2025 Miriam Santana: No considerar la sucursal para obtener todos los movimientos del invetario y enviarla en los xml para mostrarla en el reporte
declare
...
begin
	...
	if (substring(trim(inventario),8,1)='N') then
		sql_datos_inv := format('select inv.c1 as sucursal_movto, inv.c3 as vehiculo, inv.c4 as vehiculo_desc,
		...
		where inv.c2=%1$L and inv.c5=%2$L and inv.c31>0', inventario, serie );												--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato, y que el inv no este cancelado
	else 
		if substring(trim(inventario),8,1)='U' then
		sql_datos_inv := format('select inv.c1 as sucursal_movto, inv.c3 as vehiculo, upper(inv.c4) as vehiculo_desc,
		inv.c5 as serie, inv.c6 as motor, inv.c8 as transmision,inv.c9 as inventario_anterior, 
		inv.c10 as color_exterior, color_ext.c4 as color_exterior_desc, inv.c13 as clave_vehicular,
...
raise notice 'PASO 1 %',datos_inv;	

	--DATOS FACTURAS COMPRA
	for sucursal_movto, partida, folio, tipo_mov, fecha,numero_cdo, clave_prov, nombre_prov, calle_prov,
	colonia_prov, poblacion_prov, rfc_prov, costo_factura_compra, iva, hold_back, importe
	in select com.c1, com.c3 , com.c4 || com.c5 || lpad(com.c6::text, 2, '0')  || lpad(com.c7::text, 3, '0')
	...
	and com.c7=mov.c5 and com.c8=mov.c6 where /*com.c1=sucursal_id and*/ com.c2=inventario order by com.c3		--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato
	loop				
		...
		<r%1$s>
			...
			<sucursal_movto>%16$s</sucursal_movto>
		...
		costo_factura_compra, iva, hold_back, importe,sucursal_movto));	--MSS 10042025 Enviar la sucursal en el xml
	
	end loop;
raise notice 'PASO 1.1.1';	

	--DATOS PEDIDO
	sql_pedido := format('select ped.c1 as sucursal_movto, ped.c3 || ped.c4 || lpad(ped.c5::text, 2, ''0'')  ||  lpad(ped.c6::text, 3, ''0'')||
	...
	where /*ped.c1=%1$L and*/ ped.c2=%1$L',/*sucursal_id,*/  inventario);		--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato
	select query_to_xml(sql_pedido, false, true, '' ) :: xml into datos_pedido;
raise notice 'PASO 2';


	--DATOS FACTURAS VENTA
	for sucursal_movto, partida, folio, tipo_mov, fecha, clave_cli, nombre_cli, calle_cli, colonia_cli, 
	poblacion_cli, rfc_cli, importe, iva, isan
	in select ven.c1, ven.c3, ven.c4 || ven.c5 || lpad(ven.c6::text, 2, '0')  
	...
	and ven.c5=mov.c3 and ven.c6=mov.c4 and ven.c7=mov.c5 and ven.c8=mov.c6 where /*ven.c1=sucursal_id 		--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato
	...
	loop 
		...
		<r%1$s>
			...
			<total>%14$s</total>
			<sucursal_movto>%15$s</sucursal_movto>
		</r%1$s>', partida, folio, tipo_mov, fecha,
		clave_cli, replace(nombre_cli,'&',''), calle_cli, colonia_cli, 
		poblacion_cli, rfc_cli, (importe -iva-isan), 
		isan,iva, importe, sucursal_movto));								--MSS 10042025 Enviar la sucursal en el xml
...
raise notice 'PASO 3';
	...
	--DATOS FACTURAS PVA
	for sucursal_movto, partida, folio, tipo_mov ,fecha ,
	clave_cli, nombre_cli, calle_cli,colonia_cli, poblacion_cli, rfc_cli,
	gastos_adm,accesorios_pva,garantia_extendida_pva, intereses_pva,
	cobranza_pva, iva, total 
	in select pva.c1, pva.c3, pva.c4 || pva.c5 || lpad(pva.c6::text, 2, '0')  
	...
	and pva.c5=mov.c3 and pva.c6=mov.c4 and pva.c7=mov.c5 and pva.c8=mov.c6 where /*pva.c1=sucursal_id and*/ pva.c2=inventario		--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato
	loop 
		...
		<r%1$s>
			...
			<sucursal_movto>%18$s</sucursal_movto>
		</r%1$s>', partida, folio, tipo_mov ,fecha,
		...
		cobranza_pva, iva, total, sucursal_movto));									--MSS 10042025 Enviar la sucursal en el xml
...
raise notice 'PASO 4';
	...
	for sucursal_movto, folio, fecha,clave_cli, nombre_cli, calle_cli, colonia_cli, poblacion_cli, rfc_cli, iva, total
	in select dsc.c1, dsc.c3 || dsc.c4 || lpad(dsc.c5::text, 2, '0')  || lpad(dsc.c6::text, 3, '0')
	...
	where /*dsc.c1=sucursal_id and*/ dsc.c2=inventario 								--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato
	loop 
		...
		<r%1$s>
			...
			<sucursal_movto>%13$s</sucursal_movto>
		</r%1$s>', contador, folio, fecha,clave_cli, replace(nombre_cli,'&',''), calle_cli,
		colonia_cli, poblacion_cli, rfc_cli, total-iva, iva, total, sucursal_movto));	--MSS 10042025 Enviar la sucursal en el xml
...
raise notice 'PASO 5';
	...
	for sucursal_movto, folio, fecha,clave_cli, nombre_cli, calle_cli, colonia_cli, poblacion_cli, rfc_cli, isan, iva, total
	in select ncred.c1, ncred.c3 || ncred.c4 || lpad(ncred.c5::text, 2, '0')  || lpad(ncred.c6::text, 3, '0')
	...
	where /*ncred.c1=sucursal_id and*/ ncred.c2=inventario 								--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato
	loop 
		...
		<r%1$s>
			...
			<sucursal_movto>%14$s</sucursal_movto>
		</r%1$s>', contador, folio, fecha,clave_cli, replace(nombre_cli,'&',''), calle_cli,
		colonia_cli, poblacion_cli, rfc_cli, total-iva-isan,isan, iva, total, sucursal_movto));	--MSS 10042025 Enviar la sucursal en el xml
...
raise notice 'PASO 6';	
	...
	for sucursal_movto, folio, tipo_mov, fecha in select  c1, c2 || c3 || lpad(c4::text, 2, '0')  || lpad(c5::text, 3, '0') || '-' || c6, 
	case when c10='0' then 'ALTA' else 'BAJA' end,c7 from keplersc.kdcomismov where /*c1=sucursal_id and*/ c8=inventario 	--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato
	loop 
		...
		<r%1$s>
			...
			<sucursal_movto>%5$s</sucursal_movto>
		</r%1$s>', 
		contador, folio, tipo_mov, fecha, sucursal_movto));							--MSS 10042025 Enviar la sucursal en el xml
...
raise notice 'PASO 7';
	...
	for sucursal_movto, clave_cli, factura, partida, cargos_xg, abonos_xg , estatus, fecha_vencimiento 
	in select ven.c1, xg.c3,xg.c4,xg.c5,xg.c6,xg.c7,xg.c10,xg.c12 from keplersc.kdventas as ven 
	inner join keplersc.kduxg as xg on ven.c1=xg.c1 and ven.c4=xg.c2 and ven.c11=xg.c3 
	and ven.c8=xg.c4 where /*ven.c1=sucursal_id and*/ ven.c2=inventario and ven.c4='U' and ven.c10=0	--MSS 10042025 No usar sucursal para obtener los datos y obtenerla como dato
	loop 
		...
		for sucursal_movto, folio, tipo ,naturaleza, fecha_expedicion, fecha_vencimiento, importe  in select xe.c1, xe.c5 || xe.c6 || 
		...
		where xe.c1=sucursal_movto and xe.c2=clave_cli and xe.c3=factura and xe.c4=partida order by xe.c11			--MSS 10042025 Usar la sucursal del movto del for anterior aqui tego duda si si o la sucursal_movto
		loop 
			...
			<r%1$s>
				...
				<sucursal_movto>%9$s</sucursal_movto>
			</r%1$s>', 
			contador_2 , folio, tipo , fecha_expedicion, fecha_vencimiento, cargo_xe, abono_xe, saldo_total, sucursal_movto));	--MSS 10042025 Enviar la sucursal en el xml
...
raise notice 'PASO 8';	
	...
	for sucursal_movto, folio, tipo, fecha, costo, tipo_mov in select inv.c1, inv.c5 || inv.c6 || lpad(inv.c7::text, 2, '0')  || lpad(inv.c8::text, 3, '0') || '-' || inv.c9, 
	...
	where /*inv.c1=sucursal_id and*/ inv.c2=inventario 									--MSS 10042025 Usar la sucursal del movto del for anterior aqui tego duda si si o la sucursal_movto
	loop 
		...
		<r%1$s>
			...
			<sucursal_movto>%7$s</sucursal_movto>
		</r%1$s>', 
		contador, folio, tipo, fecha, entrada_mov_inv, salida_mov_inv, sucursal_movto));	--MSS 10042025 Enviar la sucursal en el xml
...
raise notice 'PASO 9';
	...
	--CARGOS AL COSTO  
	for sucursal_movto, folio , tipo, fecha, tipo_costo, genero, naturaleza, costo  in select cst.c1, cst.c2 || cst.c3 || lpad(cst.c4::text, 2, '0')  
	...
	and cst.c3=mov.c3 and cst.c4=mov.c4 and cst.c5=mov.c5 and cst.c6=mov.c6 where cst.c7=sucursal_id and cst.c8=inventario order by cst.c1, mov.c9 		--MSS 10042025 Usar la sucursal del movto del for anterior aqui tego duda si si o la sucursal_movto
	loop 
		...
		contador,folio, tipo, fecha, cargo, abono, saldo_total, tipo_costo, sucursal_movto));		--MSS 10042025 Enviar la sucursal en el xml
