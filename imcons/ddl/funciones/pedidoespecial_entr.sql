CREATE OR REPLACE FUNCTION keplersc.pedidoespecial_entr(dataxml xml)
...
begin
	...
	if upper(operacion) <> 'ENTREGA' then 
	...
	*/
	...
	if totReg = 0 then
		...
		intValor := 0;
		select c14 into intValor from keplersc.kdserped c where c1 = sucursal and c2 = k_folio; 
		intValor := coalesce(intValor, -1);
		if intValor = 10 then
			mensaje := 'El Pedido Especial [ ' || K_folio || ' ]  ,  ya esta Totalmente Entregado ...';
			raise exception '%', mensaje;	
		end if;
	end if;
	...
	*/
	...
	select sum(1), sum(case when c10 = 10 then 1 else 0 end) into totReg, totEntr 
	...
	if totReg = 0 then
		...
		if totReg = totEntr then
			mensaje := 'El Pedido Especial  [ ' || K_folio || ' ]  ,  No tiene partidas disponibles para Entregarse ...';
			raise exception '%', mensaje;	
		end if;
	end if;
	...
	*/
	...
	for intCont in 0..no_partidas - 1 loop
		...
		entr_fech := coalesce((xpath('//document/k_mov/r'||intCont||'/k_entr/text()',dataxml))[1],'');
		entr_st := coalesce((xpath('//document/k_mov/r'||intCont||'/k_st_entr/text()',dataxml))[1],'');
	
		c_tipo := coalesce((xpath('//document/k_mov/r'||intCont||'/k_r_tipo/text()',dataxml))[1],'');
		c_orden := coalesce((xpath('//document/k_mov/r'||intCont||'/k_r_orden/text()',dataxml))[1],'');
		...
		if no_vacios > 0 then
			...
			*/
			...
			if length(entr_fech) > 0 and (length(entr_st) = 0 or entr_st::int = 0) and (length(c_tipo) = 0 or length(c_orden) = 0) then 
				mensaje := 'El Tipo y/o la Orden son datos obligatorios para registrar la partida como entregada ... Parte [' || parte || '] , Partida [' || right((intCont+1)::text,3) || ']';
				raise exception '%', mensaje;	
			else
			
				if length(entr_fech) > 0 and (length(entr_st) = 0 or entr_st::int = 0) then 
				
					-- validar datos asociados a la entrega (tipo, orden)
					totReg := 0;
					select count(c1) into totReg from keplersc.kdmargen where c1 = c_tipo;
					if totReg = 0 then
						mensaje := 'No se encontro el Registro en la Tabla kdmargen [Tipo Orden] [' || c_tipo || ']';
						raise exception '%', mensaje;
					else
						mensaje := '';
					end if;	
					...
					totReg := 0;
					select count(c1) into totReg from keplersc.kdord where c1 = sucursal and c2 = c_tipo and c3 = c_orden;
					if totReg = 0 then
						mensaje := 'No se encontro el Registro en la Tabla kdord [Tipo Orden, Orden] [ ' || c_tipo || ' , ' || c_orden || ' ]';
						raise exception '%', mensaje;
					else
						mensaje := '';
					end if;	
	...
	if upper(operacion) = 'ENTREGA' then
		...
		for intCont in 0..no_partidas - 1 loop 
			...
			*/
			...
			if cant_d <= 0 then 
				...
				-- UPD {kdserpedmov} 
				-- Condition Updated 20231226 in order to align this to free input on grid
				if length(entr_fech) > 0 /*and (length(entr_st) = 0 or entr_st::int = 0)*/ then 
					totReg := 0;
					...
					if totReg = 0 then
						...
						set c10 = 10, c11 = to_date(current_date::text,'YYYY-MM-DD'), c12 = c_tipo, c13 = c_orden  
						where c1 = sucursal and c2 = k_folio and c3 = numero_partida and c4 = item_part;
					end if;
				end if;
		...
		where c1 = sucursal and c2 = k_folio and c10 = 0;
		totReg = coalesce(totReg, -1);
		if totReg <> 0 then
			if totReg < 0 then
				mensaje := 'No se pudo verificar el Estatus de Entrega del Pedido Especial [ ' || k_folio || ' ] ';
				raise exception '%', mensaje;
			end if;
		else
	...
	/*
	mensaje := 'Opcion [Pedido Especial - Entregado] en pruebas operativas ...';
	...
	*/
	...
	resultado := '1';
	if upper(operacion) = 'ENTREGA' then
		adicionales/*mensaje*/ := 'Se Actualizo el Estatus de Entrega del Pedido Especial [ ' || k_folio || ' ]  ...';
...
exception
	when others then
		resultado := '0';
		mensaje := 'pedidoespecial_entr(); ' || '['|| sqlstate || '] ' || sqlerrm ;
		return query select resultado, mensaje, adicionales;	
