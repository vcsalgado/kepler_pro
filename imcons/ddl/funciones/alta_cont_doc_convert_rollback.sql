CREATE OR REPLACE FUNCTION keplersc.alta_cont_doc_convert_rollback(dataxml xml, xmlkdm1 xml, xmlkdmm xml, folio_operacion text)
...
exception
	when others then
		resultado := 0;
		mensaje := 'alta_cont_doc_convert_rollback() ' || '['|| sqlstate || '] ' || sqlerrm;
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
end;
$function$
