CREATE OR REPLACE FUNCTION keplersc.baja_cont_doc_no_deduc(dataxml xml, folio_operacion text)
...
begin
	...
	totalReg := 0;
	select count(*) into totalReg from keplersc.kdmdocsnodeduc  
	where c1 = sucursal_id and c2 = genero and c3 = naturaleza and c4 = grupo::int and c5 = tipo_clave::int
		and c6 = folio_operacion;
	if totalReg = 0 then
		mensajeError := 'No se encontraron Registros a Revertir en la Tabla { kdmdocsnodeduc } ...';
		raise exception '%',mensajeError;			
	end if;


	delete from keplersc.kdmdocsnodeduc where c1 = sucursal_id and c2 = genero and c3 = naturaleza and c4 = grupo::int and c5 = tipo_clave::int 
		and c6 = folio_operacion;
	...
	if abs(totalReg) > 0 then
		mensajeError := 'Se encontraron Registros Inconsistentes al Eliminar Registros en la Tabla { kdmdocsnodeduc } ...';
		raise exception '%',mensajeError;			
	end if;
...
exception
	when others then
		resultado := 0;
		mensaje := 'baja_cont_doc_no_deduc() ' || '['|| sqlstate || '] ' || sqlerrm ;
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
end;
$function$
