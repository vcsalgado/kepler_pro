CREATE OR REPLACE FUNCTION keplersc.invr_actualizar_estadistica_ifis(sucursal_proceso text)
 RETURNS TABLE(resultado text, mensaje text, adicionales text)
 LANGUAGE plpgsql
AS $function$
declare
	/*
	 * Descripcion: Recalcula las estadisticas de prouctos solo de tabla kdifis
...
--raise exception 'Funcion deshabilitada';
	...
	--Iniciar tablas de estadisticas dedicadas a proceso
	truncate table keplersc.ifis_kdink;
	truncate table keplersc.ifis_kdinl;
	truncate table keplersc.ifis_kdreflastmov;

	--Iniciar tablas de estadisticas con productos
	insert into keplersc.ifis_kdink (c1,c2,c3) values('00','0', '0000'); --registro dummy temporal
	insert into keplersc.ifis_kdinl (c1,c2) values('00','0'); --registro dummy temporal

	producto_ant = '';
	for sucursal_id, producto_act, anio_actual  in 
		select distinct c1,c2,extract(year from c3 )::text from keplersc.kdinm inm 
		where inm.c1=sucursal_proceso and inm.c2 in (select fis.c2 from keplersc.kdifis fis where fis.c1=inm.c1 and fis.c2=inm.c2) 
		order by c1,c2 
	loop
raise notice 'Procesando parte:%', producto_act;
raise notice 'PASO 1 en Suc:%, Prod.:%, Fecha:%',sucursal_id,producto_act,anio_actual;	
		...
		select * into rec_KDINK from keplersc.ifis_kdink where c1='00';
		if producto_act <> producto_ant then
			if producto_ant <> '' then
				insert into keplersc.ifis_kdinl(c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,c18,c19,c20)
			...
			select * into rec_KDINL from keplersc.ifis_kdinl where c1='00';
...
raise notice 'PASO 2 en Suc:%, Prod.:%, Fecha:%',sucursal_id,producto_act,fecha_ult_mov;	
		...
		insert into keplersc.ifis_kdink(c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,c18,c19,c20,
	...
	if producto_act <> '' then
		...
		insert into keplersc.ifis_kdreflastmov(c1,c2,c3) values(sucursal_id,producto_act,fecha_ult_mov);		
	end if;
	delete from keplersc.ifis_kdink where c1='00';
	delete from keplersc.ifis_kdinl where c1='00';

	--Actualizar tablas de estadisticas de los productos recalculados
	delete from keplersc.kdinl inl where inl.c2 in(select ifisinl.c2 from keplersc.ifis_kdinl ifisinl where ifisinl.c1=inl.c1 and ifisinl.c2=inl.c2 );
	insert into keplersc.kdinl select * from keplersc.ifis_kdinl;

	delete from keplersc.kdink ink where ink.c2 in(select ifisink.c2 from keplersc.ifis_kdink ifisink where ifisink.c1=ink.c1 and ifisink.c2=ink.c2 );
	insert into keplersc.kdink select * from keplersc.ifis_kdink;

	delete from keplersc.kdreflastmov mov where mov.c2 in(select ifismov.c2 from keplersc.ifis_kdreflastmov ifismov  where ifismov.c1=mov.c1 and ifismov.c2=mov.c2 );
	insert into keplersc.kdreflastmov select * from keplersc.ifis_kdreflastmov;
