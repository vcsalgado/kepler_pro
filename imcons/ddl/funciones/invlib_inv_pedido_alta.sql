CREATE OR REPLACE FUNCTION keplersc.invlib_inv_pedido_alta(dataxml xml, folio_operacion text)
...
begin
	...
	tipo_operacion := coalesce((xpath('//document/k_tipo_operacion/r2/text()', dataxml))[1]::text,'')::text;--c97
	...
	select c41,c42,c40,c86,c98,c87,c46 into v_fecha_referencia,	v_saldo_documento,v_paridad,v_dias_retraso,v_iva_desglosado,v_tasa_interes_moratorio,
			v_clave_cliente_proveedor from keplersc.KDM1 where c1 = v_sucursal_id and 
...
exception
	when others then
		resultado := 0;
		mensaje := 'invlib_inv_pedido_alta() ' || '['|| sqlstate || '] ' || sqlerrm ;
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
end;
$function$
