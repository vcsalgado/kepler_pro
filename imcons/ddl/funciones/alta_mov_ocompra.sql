CREATE OR REPLACE FUNCTION keplersc.alta_mov_ocompra(dataxml xml, folio_operacion text)
 RETURNS TABLE(resultado text, mensaje text, adicionales text)
 LANGUAGE plpgsql
AS $function$
--Descripcion: Realiza inserción de partidas de una Orden Compra en KDM6
--Autor: Luis Leal
--Fecha: 23/02/23
declare
...
begin
	...
	for cont in 0..no_partidas - 1 loop
		...
		if clave_gasto = '' or desc_gasto = '' or monto_gasto = '' then
			raise exception 'Faltan datos para la partida numero: %' , numero_partida;
		end if;
		
		insert into keplersc.kdm6 (c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11)
...
exception
	when others then
		resultado := 0;
		mensaje := 'alta_mov_ocompra() ' || '['|| sqlstate || '] ' || sqlerrm ;
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
end;
$function$
