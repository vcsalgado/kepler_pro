CREATE OR REPLACE FUNCTION keplersc.alta_doc_sec(dataxml xml, folio_operacion text)
 ...
 LANGUAGE plpgsql
AS $function$
--Descripcion: Realiza insercion de movimientos en KDM5
--Autor: Luis Leal
--Fecha: 18/10/22
--04/03/2024 (JMM) :
...
---- para las Operaciones de Nvo Esquema de CxP - Pagos Transfers {NEW DOC TYPE}
---30/09/2025 VCSS Se integran Impuestos adicionales
...
declare
	...
	--VCSS 30/09/2025 Impuestos
	uuid_subtotal numeric = 0;
	uuid_total numeric = 0;
	uuid_trasiva numeric = 0;
	uuid_trasieps numeric = 0;
	uuid_retiva numeric = 0;
	uuid_retisr numeric = 0;
	uuid_otroimptoa numeric = 0;
	uuid_otroimptob numeric = 0;
...
begin
	...
	for cont in 0..no_partidas - 1 loop
		...
		if factura is null or factura = '' then
			continue;
		end if;
		...
		--VCSS 30/09/2025 Impuestos
		uuid_subtotal := coalesce((xpath('//document/k_subtotal/text()',dataxml))[1]::text,'0')::numeric;
		uuid_total := coalesce((xpath('//document/k_mov/r' || cont || '/k_monto_factura/text()',dataxml))[1]::text,'0')::numeric;
		uuid_trasiva := coalesce((xpath('//document/k_mov/r' || cont || '/k_iva_factura/text()',dataxml))[1]::text,'0')::numeric;
		uuid_trasieps := coalesce((xpath('//document/k_mov/r' || cont || '/k_ieps_tras/text()',dataxml))[1]::text,'0')::numeric;
		uuid_retiva := coalesce((xpath('//document/k_mov/r' || cont || '/k_iva_ret/text()',dataxml))[1]::text,'0')::numeric;
		uuid_retisr := coalesce((xpath('//document/k_mov/r' || cont || '/k_isr_ret/text()',dataxml))[1]::text,'0')::numeric;
		uuid_otroimptoa := coalesce((xpath('//document/k_mov/r' || cont || '/k_imptoadica/text()',dataxml))[1]::text,'0')::numeric;
		uuid_otroimptob := coalesce((xpath('//document/k_mov/r' || cont || '/k_imptoadicb/text()',dataxml))[1]::text,'0')::numeric;
		...
		insert into keplersc.kdm5 (c1,c2,c3,c4,c5,c6,c7,c11,c12,c13,c14, c15, c16
			,cve_prov_oper, cve_prov_pago, cta_deposito, doc_refer_compl
			,subtotal,total,trasiva,trasieps,retiva,retisr,otroimptoa,otroimptob)
		values(sucursal_id,genero,naturaleza,grupo::integer,tipo_clave::integer,
			...
			, clave_prov_oper, clave_prov_pago, cta_dep, ref_compl
			,uuid_subtotal,uuid_total,uuid_trasiva,uuid_trasieps,uuid_retiva,uuid_retisr,uuid_otroimptoa,uuid_otroimptob);
	end loop ;	
