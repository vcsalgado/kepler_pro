...
begin
	...
	flag_factura_sust:=coalesce((xpath('//document/ambiente/flag_factura_sust/text()',dataxml))[1]::text,'')::text;		--25022025 Factura por sustitucion
	genero_docto := coalesce((xpath('//document/k_gendocto/text()',dataxml))[1]::text,'')::text;
	...
	--raise exception 'v_operacion:%',v_operacion;
			if v_operacion = 'G' then --FACTURA					
				if strValor::int < 80 then --'VALIDA TODO EL PROCESO DE VENTA MIENTRAS NO SEA TRASPASO						
					iF strValor::int = 10 and v_estado_compra >= 20 then --B11001=0:
						adicionales := 'La compra para este inventario, ya esta registrada'; 
						raise exception '%',adicionales; 
					end if;
					
					if strValor::int>=20 and v_estado_compra < 20 then 
						adicionales := 'No se puede realizar esta operación, porque la compra todavia no esta dada de alta'; 
						raise exception '%',adicionales;
					end if;
					
					if strValor::int = 20 and v_estado_venta >= 10 then 
						adicionales := 'El pedido para este inventario, ya esta registrado'; 
						raise exception '%',adicionales;
					end if;

					if strValor::int > 20 and v_estado_venta < 10 then 
						adicionales := 'No se puede realizar esta operación, porque el pedido todavia no esta dado de alta'; 
						raise exception '%',adicionales;
					end if;
					--MSS 25022025 No validar si es factura x sustitucion, aqui validar si hay una factura pendiente por cancelar, sólo debe haber 2 vivas					
					if flag_factura_sust='' or flag_factura_sust is null then
						if strValor::int = 30 and v_estado_venta >= 20 then 
							adicionales := 'La factura para este inventario, ya esta registrada'; 
							raise exception '%',adicionales;
						end if;
					else
						select * into get_resultado,get_mensaje,get_adicionales
							from keplersc.verify_es_factura_sustituible(v_sucursal_id,genero_docto,naturaleza_docto,grupo_docto,tipo_docto,folio_docto);
						if get_resultado='0' then
							adicionales:=get_mensaje;
							raise exception '%',adicionales;
						end if;
					end if;
					if strValor::int = 30 and ((select EXTRACT(year  FROM v_fecha::timestamp))::numeric > (select EXTRACT(year FROM (select now()-interval '1 day')))::numeric and
						...
						adicionales := 'La fecha de la Factura no puede ser posterior a hoy'; 
						raise exception '%',adicionales;
					end if;
				
					if strValor::int > 30 and v_estado_venta < 20 then 
						adicionales := 'No se puede realizar esta operación, porque la factura todavia no esta dada de alta'; 
						raise exception '%',adicionales;
					end if;
					
					if strValor::int = 70 and v_estado_venta >= 60 then 
						adicionales := 'El vale de salida para este inventario, ya esta registrado'; 
						raise exception '%',adicionales;
					end if;
				...
				--MSS 010523 Codigo faltante en la funcion
                    ...
                    if  strValor::int = 80 then  --TRASPASOS
                        if  v_estado_compra < 20 then  
                            adicionales := 'No se puede efectuar el traspaso, debido a que no esta realizada la compra'; 
							raise exception '%',adicionales;
                        end if;
                        if v_estado_venta > 0 then  
                            adicionales := 'No se puede efectuar el traspaso, debido a que la unidad no esta en inventario'; 
							raise exception '%',adicionales;
                        end if;
                    else  --factura de PVA o Nota de Descuento
                        if  v_estado_venta < 20 then  
                            adicionales := 'No se pueden hacer Notas de Descuento o Facturas de PVA hasta que se realice la factura de la Unidad'; 
							raise exception '%',adicionales;
                        end if;
                        if  v_estado_venta = 60 then  
                            adicionales := 'No se pueden hacer Notas de Descuento o Facturas de PVA si ya esta realizado el Vale de Salida'; 
							raise exception '%',adicionales;
                        end if;
                        if  v_estado_venta = 70 then
                        	adicionales := 'No se pueden hacer Notas de Descuento o Facturas de PVA si la unidad fue traspasada a otra agencia'; 
							raise exception '%',adicionales;
                        end if;
                    end if;	                
				...
				if strValor::int < 80 then --VALIDA TODO EL PROCESO DE BAJA DE VENTA MIENTRAS NO SEA TRASPASO						
					if strValor::int = 10 and v_estado_compra = 10 then 
						adicionales := 'La compra para este inventario, ya esta cancelada'; 
						raise exception '%',adicionales;
					end if;
				
					if strValor::int = 30 and v_estado_venta < 20 then 
						adicionales := 'La factura para este inventario, ya esta cancelada'; 
						raise exception '%',adicionales;
					end if;
				
					if strValor::int <= 10 and v_estado_venta >= 10 then 
						adicionales := 'Para realizar este movimiento primero hay que dar de baja el Pedido'; 
						raise exception '%',adicionales;
					end if;
					
					if strValor::int <= 20 and v_estado_venta >= 30 then 
						adicionales := 'Para realizar este movimiento primero hay que hacer la Nota de Credito de la Factura'; 
						raise exception '%',adicionales;
					end if;
					
					if strValor::int <= 30 and v_estado_venta >= 60 then 
						adicionales := 'Para realizar este movimiento primero hay que dar de baja el Vale de Salida'; 
						raise exception '%',adicionales;
					end if;
					...
					*/
					...
					if v_estado_venta = 70 then
						adicionales := 'Para realizar este movimiento primero hay que elaborar la Nota de Credito de la Factura de Traspaso'; 
						raise exception '%',adicionales;
					end if;
				else --NOTA DE CREDITO DE TRASPASOS Y MOVIMIENTOS FUERA DE OPERACION
					if strValor::int = 80 then
						if v_estado_venta <> 70 then
							adicionales := 'No se puede efectuar la baja el traspaso, por 2 razones: 1-Ya esta dada de baja previamente. 2- Se encuentra en el proceso de Ventas.'; 
							raise exception '%',adicionales;	
						end if;
					else --Nota de Credito de PVA o Cancelacion de Nota de Descuento
						if v_estado_venta = 60 then
							adicionales := 'No puede hacer la Nota de Credito de PVA o la Cancelacion de la Nota de Descuento"," porque no se ha dado de baja el vale de salida'; 
							raise exception '%',adicionales;	
						end if;
						if v_estado_venta = 70 then
							adicionales := 'No puede hacer la Nota de Credito de PVA o la Cancelacion de la Nota de Descuento"," porque no ha hecho la Nota de Credito del Traspaso de la Unidad'; 
							raise exception '%',adicionales;	
						end if;
					end if;
				end if;
			...
			adicionales := 'No se encuentra el Numero de Inventario, Imposible Continuar';
			raise exception '%',adicionales;
		end if; --end KDINF	
	end if;
	resultado ='1';
