CREATE OR REPLACE FUNCTION keplersc.alta_cont_doc_convert(dataxml xml, xmlkdm1 xml, xmlkdmm xml, folio_operacion text)
...
begin
	...
	*/
	...
	if upper(flag_contrarec) <> 'CXP_CONTR_REC_CONVERT' then 
		raise exception '%', 'Se esta llamando a la funcion [ alta_cont_doc_convert ] desde una Operacion No Valida ...';
	else
		var_st_compr = 'X';
	end if;
	...
	if (xpath('//row/c6/text()', xmlKDMM))[1]::text  = 'S' and (xpath('//row/c71/text()', xmlKDMM))[1]::text  = 'S' then 
		...
		for partida,clave_cuenta,descr_cuenta,tipo_asiento_kdc,monto,inventario ,afectacion/*Added by JMM 20240308*/ 
			...
			in select c7,c8,c9,c10,c11,c13 ,c12/*Added by JMM 20240308*/ ,ctopto/*Added by JMM 20240723*/ from keplersc.kdmdocscompr 
...
exception
	when others then
		resultado := 0;
		mensaje := 'alta_cont_doc_convert() ' || '['|| sqlstate || '] ' || sqlerrm;
		adicionales := '';
		return query select resultado, mensaje, adicionales;	
end;
$function$
