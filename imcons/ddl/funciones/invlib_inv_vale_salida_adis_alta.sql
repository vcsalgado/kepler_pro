CREATE OR REPLACE FUNCTION keplersc.invlib_inv_vale_salida_adis_alta(dataxml xml, xmlkdmm xml, folio_operacion text)
 RETURNS TABLE(resultado text, mensaje text, adicionales text)
 LANGUAGE plpgsql
AS $function$
declare
	--Autor: Saltiel RC. 07/NOV/2022
...
begin
	...
	if (select count(*) from keplersc.kdpedido where c1 = v_sucursal_id and c2 = v_inventario) > 0 then 
		select c15 into v_subsidio from keplersc.kdpedido WHERE c1 = v_sucursal_id and C2 = v_inventario;
		for v_naturaleza_KDBONIF,v_monto_KDBONIF in select c3,c10 from keplersc.KDBONIF where c1 = v_sucursal_id and c8 = v_inventario
		loop
				if v_naturaleza_KDBONIF = 'D' then 
					v_Cargos_de_Bonificaciones = v_Cargos_de_Bonificaciones + v_monto_KDBONIF;
				else
					v_Abonos_de_Bonificaciones= v_Abonos_de_Bonificaciones+v_monto_KDBONIF;
				end if;
		end loop;
		for v_genero_KDSUNICOSTO,v_naturaleza_KDSUNICOSTO, v_tipo_costo_KDSUNICOSTO, v_costo_KDSUNICOSTO in select c2,c3,c9,c10  from keplersc.KDSUNICOSTO where c7 = v_sucursal_id and c8 = v_inventario
		loop
		...
		values (v_sucursal_id,genero,naturaleza,grupo::numeric,tipo_clave::numeric,
				folio_operacion,0,v_subsidio,v_Cargos_de_Bonificaciones,
